...
const int ALERT_VIBRATOR_PIN_1 = 0 ;  // *** NEW: Pin for the Vibrator Motor (e.g., GPIO 3) ***
const int ALERT_VIBRATOR_PIN_2 = 4 ;  // *** NEW: Pin for the Vibrator Motor (e.g., GPIO 4) ***
...
void setVibratorPins(int value ) {
  digitalWrite(ALERT_VIBRATOR_PIN_1, value);
  digitalWrite(ALERT_VIBRATOR_PIN_2, value);
}
...
void setup() {
  ...
  pinMode(ALERT_VIBRATOR_PIN_1, OUTPUT);
  pinMode(ALERT_VIBRATOR_PIN_2, OUTPUT);
  setVibratorPins(LOW);
...
float read_uv_index(int pin) {
  ...
  float uv_index = (sensor_millivolts / 3300.0) * 4 * 15;
  return uv_index;
}
